...
while selected=$(list_entries | xargs -I {} bash -c "print_entry {}" | column -t | $ROFI_CMD -p "Autostart" -selected-row ${selected_row} -format 'i s'); do
	selected_row=$(echo "$selected" | awk '{print $1;}')
    selected_text=$(echo "$selected" | cut -d' ' -f2-)
	selected_entry=$(echo $selected_text | cut -f1 -d" ")

	action=$(gen_menu "$selected_text" | $ROFI_CMD -p "$selected_entry")
